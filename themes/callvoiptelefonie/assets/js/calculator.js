...
const renderMonthlyTotal = function () {
  ...
  const maxValue = Math.max(...['toestelaccounts', 'bellen-via-de-computer', 'bellen-met-de-smartphone'].map(e => document.getElementById(e).valueAsNumber)) || 1
  ...
  <div class="text-center mt-4 p-4 rounded-md border-2 border-grey-dark" id='single-person-price'><span class='font-medium'> ${totalMaxValue}</span> <span>per gebruiker per maand</span></div>
  <br>
  `;
...
const renderTemplateRow = function (fields, type) {
  ...
  ${item.value > 0
            ? `
  <div class="flex mb-1">
    <div class="w-8">${item.value} x</div>
...
const renderCallminutesFields = function (fields) {
  ...
  const numberOfDevices = document.getElementById("toestelaccounts").value,
    qallerPlus = document.getElementById("bellen-met-de-smartphone").value,
    webcallAccounts = document.getElementById("bellen-via-de-computer").value;
...
const renderVamosFields = function (fields) {
  const vamosFieldsMonthly = document.getElementById("options-fields-monthly");
  const vamosFieldsOnetime = document.getElementById("options-fields-onetime");

  const selected = fields[0];
  let monthly = parseFloat(selected.options[selected.selectedIndex].dataset.price_monthly) || 0;
  let onetime = parseFloat(selected.options[selected.selectedIndex].dataset.price_onetime) || 0;

  const value = selected.value;
  const name = selected.options[selected.selectedIndex].dataset.name;

  const vamosItem = [{
    name: name,
    price_monthly: monthly,
    price_onetime: onetime,
    value: 1
  }];

  state.optionsFields = [...state.optionsFields, ...vamosItem];

  state.monthly.optionsFields += monthly;
  state.onetime.optionsFields += onetime;

  vamosFieldsMonthly.innerHTML += renderTemplateRow(vamosItem, "monthly");
  vamosFieldsOnetime.innerHTML += renderTemplateRow(vamosItem, "onetime");
};
...
const renderRows = function () {
  ...
  if (document.getElementById("vamos-fields")) {
    const vamosFields = document.getElementById("vamos-fields").getElementsByTagName("select");
    renderVamosFields(vamosFields);
  }
    const fields = document
...
document.getElementById("submit-small-form").onclick = function () {
  const name = document.getElementById("s-name"),
    ...
    namev = name.value.length < 1,
    emailv = email.value.length < 1;
  namev
  ...
  document.querySelector('[name="calculator-formulier"]').submit();
};
